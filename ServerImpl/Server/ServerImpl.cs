...
namespace Server
{
    public class ServerImpl : IServer
    {
        ...
        public void setDB()
        {
            ...
            _db.addSubject(s);
            // set topic
            Topic t1 = new Topic { TopicId = "Normal", SubjectId = "Chest x-Rays", subject = s, timeAdded = DateTime.Now };
            _db.addTopic(t1);
            Topic t2 = new Topic { TopicId = "Cavitary Lesion", SubjectId = "Chest x-Rays", subject = s, timeAdded = DateTime.Now };
            _db.addTopic(t2);
            Topic t3 = new Topic { TopicId = "Interstitial opacities", SubjectId = "Chest x-Rays", subject = s, timeAdded = DateTime.Now };
            _db.addTopic(t3);
            _subjectsTopics["Chest x-Rays"] = new List<string>() { "Cavitary Lesion", "Interstitial opacities" };
/******************************/
            // set subject
            Subject s1 = new Subject { SubjectId = "test subject", timeAdded = DateTime.Now, topics = new List<Topic>() };
            _db.addSubject(s1);
            // set topic
            Topic t11 = new Topic { TopicId = "Normal", SubjectId = "test subject", subject = s1, timeAdded = DateTime.Now };
            _db.addTopic(t11);
            Topic t22 = new Topic { TopicId = "t22", SubjectId = "test subject", subject = s1, timeAdded = DateTime.Now };
            _db.addTopic(t22);
            Topic t33 = new Topic { TopicId = "t33", SubjectId = "test subject", subject = s1, timeAdded = DateTime.Now };
            _db.addTopic(t33);
            _subjectsTopics["test subject"] = new List<string>() { "t22", "t33" };
/****************************************/
            ...
            Question q1 = new Question
            {
                ...
                diagnoses = new List<string>() { "Normal" },
                timesAnswered = 0,
        ...
        public string AnswerAQuestion(int userUniqueInt, int questionID, bool isNormal, int normalityCertainty, List<string> diagnoses, List<int> diagnosisCertainties)
        {
            ...
            {
                UserLevel userLevel = _db.getUserLevel(user.UserId, q.subjectName, s);
                if (userLevel == null)
                {
                    Topic t = _db.getTopic(q.subjectName, s);
                    userLevel = new UserLevel { userId = user.UserId, SubjectId = q.subjectName, TopicId = s, level = Levels.DEFAULT_LVL, timesAnswered = 0, timesAnsweredCorrectly = 0 };
