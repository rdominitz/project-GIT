...
namespace JamesWright.SimpleHttp
{
    class Listener
    {
        ...
        private async Task<bool> TryRespondAsync(Request request, RouteRepository routeRepository)
        {
            ...
            {
                Dictionary<string, Action<Request, Response>> gets = routeRepository.GetRoutes(Methods.Get);
                await Task.Run(() =>
                {

                    gets["/404"](request, new Response(context.Response));
                });
                return false;
            }
